...
ranked_sales AS (
  SELECT 
    market_date,
    total_sales,
    RANK() OVER (ORDER BY total_sales DESC) AS rank_high, -- Highest sales first
    RANK() OVER (ORDER BY total_sales ASC) AS rank_low  -- Lowest sales first
  FROM total_sales
)
SELECT 
  market_date,
...
FROM ranked_sales
WHERE rank_high = 1  -- Select the market date with the highest sales
...
SELECT 
  ...
  'Lowest' AS sale_type
FROM ranked_sales
WHERE rank_low = 1;  -- Select the market date with the lowest sales
...
-- Create the new table `product_units`
...
FROM 
  product
WHERE 
  product_qty_type = 'unit';  -- Filter to include only products with 'unit' as qty type
